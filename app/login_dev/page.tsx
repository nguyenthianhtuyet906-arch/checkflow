...
import { Loader2, Zap, Shield, BarChart3, Users, Copy, CheckCircle, AlertTriangle } from "lucide-react"
  ...
  const handleTestSignIn = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(result.message || "Failed to sign in as test user")
      }

      const result = await response.json()
      if (result.token) {
        localStorage.setItem("auth-token", result.token) // Store token in localStorage
        await refreshUser() // Refresh user data in context
        router.push(HOME_URL) // Redirect to home URL
      } else {
        throw new Error("No token received from test login")
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Failed to sign in as test user"
      setError(errorMessage)
    } finally {
            ...
            <Card className="shadow-lg border-0 bg-blue-50 border-blue-200">
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-blue-700">
                  Copy this redirect URL to your Google OAuth app configuration
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex items-center space-x-2">
                  <code className="flex-1 p-2 bg-white rounded border text-sm text-gray-800 break-all">
                    {redirectUrl}
                  </code>
                  <Button onClick={copyRedirectUrl} size="sm" variant="outline" className="shrink-0 bg-transparent">
                    {redirectUrlCopied ? (
                ...
                {redirectUrlCopied && <p className="text-sm text-green-600 mt-2">✓ Copied to clipboard!</p>}
              </CardContent>
            </Card>
            ...
            <Card className="shadow-xl border-0 bg-white">
              ...
              <CardContent className="space-y-6">
                ...
                <Button
                  ...
                  disabled={isTestSigningIn}
                  className="w-full h-12 bg-pink-600 hover:bg-pink-700 text-white shadow-sm"
                >
                  {isTestSigningIn ? (
                    <Loader2 className="h-5 w-5 animate-spin mr-2" />
                  ) : (
                  ...
                  {isTestSigningIn ? "Signing in as Test User..." : "Login as Test User"}
                </Button>
